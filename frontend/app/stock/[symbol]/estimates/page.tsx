...
export default function EstimatesPage({params}:Props) {
  ...
  const earnings=[{q:"Q3 FY26",e:"₹18.50",a:"₹19.20",b:true},{q:"Q2 FY26",e:"₹17.80",a:"₹18.10",b:true},{q:"Q1 FY26",e:"₹16.90",a:"₹16.50",b:false},{q:"Q4 FY25",e:"₹15.20",a:"₹16.80",b:true}]
  return(
    <div className="flex min-h-screen bg-background"><StockSidebar/><div className="flex flex-1 flex-col pl-[260px]"><main className="flex-1 p-6 space-y-6">
      <div><h1 className="text-2xl font-bold">{symbol} Estimates</h1><p className="text-sm text-muted-foreground">Analyst consensus & earnings</p></div>
      ...
      <Card><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2"><TrendingUp size={20}/>Earnings History</CardTitle></CardHeader><CardContent><table className="w-full text-sm"><thead><tr className="border-b border-border">{["Quarter","Estimate","Actual",""].map(h=><th key={h} className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">{h}</th>)}</tr></thead><tbody>{earnings.map(e=><tr key={e.q} className="border-b border-border/50 hover:bg-secondary/30"><td className="px-4 py-3 font-medium">{e.q}</td><td className="px-4 py-3 text-muted-foreground">{e.e}</td><td className="px-4 py-3 font-semibold">{e.a}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${e.b?"bg-primary/10 text-primary":"bg-destructive/10 text-destructive"}`}>{e.b?"Beat":"Miss"}</span></td></tr>)}</tbody></table></CardContent></Card>
    </main></div></div>)
}
