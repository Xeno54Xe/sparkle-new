...
import { LayoutDashboard, Search, Star, LineChart, FileText, Settings, ChevronLeft, ChevronRight } from "lucide-react"
...
export function Sidebar() {
  ...
  return (
    <aside className={cn("fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-white/5 glass transition-all duration-300", collapsed ? "w-[72px]" : "w-[260px]")}>
      <div className="flex h-16 items-center justify-between border-b border-white/5 px-4">
        {!collapsed && <Link href="/" className="text-xl font-bold tracking-tight text-gradient-primary">SparkleAI</Link>}
        <button onClick={() => setCollapsed(!collapsed)} className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition-all hover:bg-white/10 hover:text-foreground">
